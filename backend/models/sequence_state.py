...
class SequenceState(Base):
    ...
    step_timestamps = Column(JSON, nullable=False, default=list)   # 各步骤命中时间戳数组
    started_at = Column(DateTime, nullable=True)   # 第一步命中时间
    expires_at = Column(DateTime, nullable=True)   # 当前步骤超时时间
